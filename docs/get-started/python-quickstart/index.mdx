...
```shell
rye add resonate-sdk==0.3.3 flask
```
...
When a request comes in, the HTTP Server extracts the URL from the request, runs the `downloadAndSummarize()` function with Resonate's `.run()`, and returns the result as JSON.
...
First, in your `app.py` file import the `RemoteServer` and pass an instance of it to the `Resonate` constructor, while providing the URL of the Resonate Server.
In this case, the URL of the server is `http://localhost:8001`.

<!--SNIPSTART quickstart-py-part-2-app {"selectedLines": ["1","7-8"]}-->
...
<!--SNIPSTART quickstart-py-part-2-app {"selectedLines": ["11-26"]}-->
...
Also, remove the registration of the function with the instance of Resonate.
...
- Import the `Scheduler` and `RemoteStore`.
