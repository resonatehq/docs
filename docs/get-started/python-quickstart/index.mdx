...
In part 3 you will build on the previous example by separating your HTTP Server from the your application code and run multiple Application Nodes to see how Resonate facilitates fan-out/fan-in use cases.
...
Notice that on subsequent requests with the same URL, Resonate does not start the executions again but returns the results immediately.
...
Therefor the `summarization()` function and `send_email()` function will be inside a loop that will continue until the summarization is accepted.
