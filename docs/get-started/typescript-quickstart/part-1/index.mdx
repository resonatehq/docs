...
title: Resonate HQ TypeScript quickstart tutorial • Part 1 🏴‍☠️
...
In the first part of this tutorial you will build a mock summarization service using Resonate HQ. This part of the tutorial showcases how Resonate provides Durable Execution for application-level failures using the SDK's local promise storage.
...
![downloadAndSummarize application flow](/img/downloadAndSummarize.svg)

Resonate will manage the execution of these functions, ensuring that the result of `download()` is passed as input to `summarize()`. In cases of transient or intermittent failures at any step, Resonate will automatically retry the execution, providing reliability to the process.
...
In the next part of the tutorial, you will connect to a Resonate Server so that the promise is stored remoted, so that even if you restart the service the functions will not re-execute and you will get the result to the original function execution.
-->
