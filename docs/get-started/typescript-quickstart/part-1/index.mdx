...
In the following diagram, **f1** is `downloadAndSummarize()`, **f2** is `download()`, and **f3** is `summarize()`.
Promises are represented by **p1**, **p2**, and **p3** respectively.

![downloadAndSummarize application flow](/img/local-downloadAndSummarize.svg)
