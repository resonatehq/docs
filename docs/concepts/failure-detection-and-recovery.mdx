...
<div style={{ maxWidth: "800px", margin: "0 auto" }}>
  <img src="/img/throw-error-for-retries.svg" alt="Throw error for retries" />
</div>
...
At the platform level, if a failure is detected, often the desired behavior is to start a new function execution on a different Application Node.

However, thanks to Durable Promises, the new function execution effectively resumes from the point of failure instead of restarting from the beginning.
...
If the timeout is reached, Resonate HQ marks the promise as failed.”
