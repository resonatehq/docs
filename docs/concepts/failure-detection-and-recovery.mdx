...
Resonate automatically retries function executions if they fail, per the specified [Retry Policy](#retry-policies), and continues to do so until the [Durable Promise Timeout](#durable-promise-timeout) is reached.
