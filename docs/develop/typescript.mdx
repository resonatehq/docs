...
➡️ [Resonate HQ TypeScript SDK API reference](https://resonatehq.github.io/resonate-sdk-ts/index.html)
...
Using [Local storage](/concepts/durable-promise#local-storage), which is the default, Resonate utilizes a volatile in-memory promise store.
...
This call graph could be visualized like the following:

<div style={{ maxWidth: "800px", margin: "0 auto" }}>
  <img src="/img/local-storage-call-graph.svg" alt="Local call graph" />
</div>

This call graph does not require the Resonate Server (supervisor).
This can be run in Local mode since all the functions execute locally within the same process.
However, if you want `f1()`, `f2()`, or `f3()` to be resumable if the process crashes, then use [Remote mode](#remote-storage).
...
Using the same code example as above, but with Remote storage, the call graph would look like the following:

<div style={{ maxWidth: "800px", margin: "0 auto" }}>
  <img src="/img/remote-storage-call-graph.svg" alt="Remote call graph" />
</div>

This call graph requires the Resonate Server (supervisor).
...
If you want to retry afunction in an attempt to [recover from an application-level](/concepts/failure-detection-and-recovery#application-level) issue, you need to either throw an error, or return a rejected promise.
